...
class MisskeyBot:
    ...
    async def _handle_mention(self, note: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"收到 @{username} 的提及: {text[:50]}{'...' if len(text) > 50 else ''}")
    ...
    async def _handle_message(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"收到用户 {user_id} 的私信: {text[:50]}{'...' if len(text) > 50 else ''}")
        ...
        try:
            ...
            for result in plugin_results:
                if result and result.get("content"):
                    ...
                    visibility = result.get("visibility", self.config.get("bot.auto_post.visibility"))
                    ...
                    logger.info(f"插件 {result.get('plugin_name')} 请求修改提示词: {plugin_prompt}")
    ...
    async def _generate_post_with_plugin(self, system_prompt: str, prompt: str, plugin_prompt: str, timestamp_override: Optional[int] = None, plugin_name: str = "system", max_tokens: int = None, temperature: float = None) -> str:
        if not prompt:
