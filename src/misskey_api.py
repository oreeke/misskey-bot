...
class MisskeyAPI:
    def __init__(self, instance_url: str, access_token: str, max_retries: int = DEFAULT_MAX_RETRIES, timeout: int = 30, config=None):
    ...
    def _is_retryable_error(self, status_code: int) -> bool:
        return status_code in {HTTP_TOO_MANY_REQUESTS, HTTP_INTERNAL_SERVER_ERROR, HTTP_BAD_GATEWAY, HTTP_SERVICE_UNAVAILABLE, HTTP_GATEWAY_TIMEOUT}
    ...
    async def create_note(self, text: str, visibility: Optional[str] = None, reply_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if visibility is None:
            if self.config:
                ...
                visibility = DEFAULT_AUTO_POST_VISIBILITY
        data = {
            "text": text,
    ...
    async def _ws_heartbeat(self) -> None:
        while True:
            try:
                ...
                logger.error(f"WebSocket 心跳网络错误: {e}")
                ...
                async for msg in self.ws_connection:
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        try:
                            ...
                            logger.error(f"无法解析 WebSocket 消息: {msg.data}")
                    elif msg.type == aiohttp.WSMsgType.ERROR:
                        logger.error(f"WebSocket 连接错误: {self.ws_connection.exception()}")
                        break
