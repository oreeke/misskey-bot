...
class PluginManager:
    ...
    def __init__(self, config: Config, plugins_dir: str = "plugins"):
        self.config = config
    ...
    async def _load_plugin(self, plugin_dir: Path, plugin_config: Dict[str, Any]) -> None:
        try:
            ...
            if spec is None or spec.loader is None:
                logger.error(f"无法加载插件规范: {plugin_dir.name}")
                return
            ...
            if plugin_class is None:
                logger.error(f"插件 {plugin_dir.name} 中未找到有效的插件类")
                return
            ...
            logger.error(f"加载插件 {plugin_dir.name} 时出错: {e}")
